...
const [count, setCount] = useState(0)
  ...
  const Like = async() => {
      if(like){
        ...
        await axios
        .delete(`${process.env.REACT_APP_API_URL}/api/heart`, 
       ...
       .catch((err) => console.log(err));
        ...
        //서버 테스트
        await axios
      .post(`${process.env.REACT_APP_API_URL}/api/heart`, 
      {
        'postId': idNum,
        'memberId': memberId 
       },
  ...
  return (
    <BodyBox>
      ...
      {like ? (<>
    <button onClick={Like}><FavoriteIcon/></button>{likeCount + count}
  </>
    ):(

    <button onClick={Like}><FavoriteBorderIcon/> 
    {likeCount + count}</button>
...
<>
{like ? (
  <button onClick={Like}><FavoriteIcon/> {likeCount+1 }개 </button>
  
):(
<button onClick={Like}><FavoriteBorderIcon/> {likeCount }개 </button>
)} 
</>
)} */}
